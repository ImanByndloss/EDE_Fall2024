...
NWIScomplete <- inner_join(NWISsites.sf, NWISflow, by=c("site_no", "station_nm"))
...
Lancaster <- NEcounties %>%
  filter(NAME == "Lancaster")

#23 Spatially select gages within the selected county
LancasterIntersects <- st_intersects(NWISsites.sf, Lancaster, 
                                     sparse = FALSE)
Lancaster.NWISsites.sf <- NWISsites.sf[LancasterIntersects,]

#24 Spatially select gages within 15 k of the selected county
LancasterBuffer <- st_buffer(Lancaster, dist = 15000)
LancasterBufferIntersects <- st_intersects(NWISsites.sf, LancasterBuffer, 
                                           sparse = FALSE)
LancasterBuffer.NWISsites.sf <- NWISsites.sf[LancasterBufferIntersects,]

#25 Plot in order of what data should be on top of the other
ggplot() +
  geom_sf(data = NEcounties, 
          color = "black", 
          fill = "grey") + 
