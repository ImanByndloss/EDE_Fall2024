...
GaringerOzone.monthly <- GaringerOzoneClean %>%
  ...
  group_by(Month,Year) %>%
  summarise(meanO3 = mean(Daily.Max.8.hour.Ozone.Concentration), .groups="keep") %>%
  mutate(FirstDay = format(make_date(Year,Month,1), "%m/%d/%Y"))
```
...
> Answer: In this case, "Seasonal" has the largest bar on the plot, indicating strong seasonal variations. To better understand the seasonality of this monthly, Seasonal Mann-Kendall (which indicated non-parametric seasonality) can be conducted.

13. Create a plot depicting mean monthly ozone concentrations over time, with both a geom_point and a geom_line layer. Edit your axis labels accordingly.

```{r}
#13
GaringerOzone.monthly$FirstDay <- as.Date(GaringerOzone.monthly$FirstDay, format="%m/%d/%Y")

# Used ggplot() to create a plot of mean monthly ozone concentrations over time.
#group=1 ensures that all data treated as one group
ggplot(GaringerOzone.monthly, aes(x=FirstDay, y=meanO3, group=1)) +
