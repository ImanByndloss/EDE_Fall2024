...
```{r project.setup, message=FALSE, error=FALSE, results='hide'}
...
# Import EPA data from the processed_KEY folder & fix dates by specifying format
epa_data <- read.csv(
...
# Compute mean PM2.5 concentration for each county, sorted by value w/ arrange()
...
# Compute mean ozone concentration for each county, sorted by value w/ arrange()
...
# $\\mu g/m^3$ (written in markdown/MathJax notation) becomes µg/m3 when knitted
...
```{r O3.2018.boxplot, echo=FALSE, fig.align='left', fig.height=2.8, fig.cap="The distribution of ozone levels per month for 2018"}
O3.2018.boxplot <- epa_data %>%
  filter(Year==2018) %>%
  drop_na(Ozone) %>%
  ggplot() + 
  aes(x=factor(Month, levels=1:12, labels=month.abb), y=Ozone) + 
  geom_boxplot() +
  labs(x="Month",y=expression(paste("Ozone (µg/", m^{3},")")))
O3.2018.boxplot
```

```{r O3.2019.boxplot, echo=FALSE, fig.align='left', fig.height=2.8, fig.cap="The distribution of ozone levels per month for 2019"}
O3.2019.boxplot <- epa_data %>%
  filter(Year==2019) %>%
  drop_na(Ozone) %>%
  ggplot() + 
  aes(x=factor(Month, levels=1:12, labels=month.abb), y=Ozone) +
...
[ImanByndloss](https://github.com/ImanByndloss/EDE_Fall2024.git)
