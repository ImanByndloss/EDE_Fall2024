...
glimpse(NTL.phys.data) # Tells the number of rows and columns as well as giving you a glimpse of the columns and records with specified type of data storage
...
# CAN ALSO BE WRITTEN AS... NTL.1999 <- filter(NTL.phys.data, year(sampledate) == "1999")

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
TL.90s <- filter(NTL.phys.data, lakename %in% c("Tuesday Lake"), year4 >= 1990 & year4<=1999)
```
Question: Why don't we filter using row numbers?

> Answer: Row numbers are not under our control and could potentially reset as the data is manipulated.
...
NTL.phys.data.processed <- 
  NTL.phys.data %>% #shift+ctrl+M adds q pipe, i.e. %>%
...
TL.July90s <- 
  ...
  filter(month(sampledate) == 7) # cannot do "07" because as it removes the month from sampledate it will become a singular number (new format)
...
meanST <- TL.July90s %>% 
  filter(depth==0) %>% 
  drop_na(temperature_C) %>% # Need to drop NA for the mean to be accurately calculated
  summarize(
    mean_surface = mean(temperature_C)) # Establishes mean_surface as the new column name; otherwise, it just says mean(temperature_C)
```
