...
date: "Fall 2024"
output: html_document # not pdf for widget, i.e., interactive rmd
runtime: shiny # also for widget
editor_options: 
...
my_plot <- nutrient_data %>% 
  ...
  labs(
    ...
    shape = "Lake", 
    color = "Depth (m)") + # does not work as "fill=" only as "color="
...
my_plot <- nutrient_data %>% 
  ...
  ggplot(aes(
    x = sampledate,
    y = .data[[the_parameter]], # to call a variable rather than a column use .data[[...]] or !!sym(...)
    color = depth,
    shape = lakename)) +
  geom_point(alpha = 0.4, size = 2) +
  scale_x_date(
...
# Input the variables of the function
 plot_it('nh34',1995,2020)
```
