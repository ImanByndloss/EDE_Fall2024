...
#Exercise 1:
G1 <- EPAair %>%
  ggplot(aes(x=Ozone,fill=as.factor(Year))) +
  geom_density(alpha=0.5) + #incorporates transparency with
  #scale_fill_manual((values=c("red","blue"))) +
  labs(x="Ozone AQI Value", y="Density", fill="")
G1                    

G2 <- EPAair %>%
  ggplot(aes(x=as.factor(Year), y=Ozone)) +
  geom_boxplot() +
...
summary(lm(data=EPAair, PM2.5 ~ Ozone))
...
Scatterplot <- ggplot(EPAair, aes(x=Ozone, y=PM2.5)) +
  geom_point() + #adds points for scatterplot
  geom_smooth(method="lm") + #adds regression line
  geom_text(x=100,y=75,label=expression("PM2.5=0.38*Ozone+15.64"))
Scatterplot
```
...
OzoneALL.reg <- lm(data=EPAair, PM2.5 ~ Ozone + Year + Month + mean_Lng + mean_Lat)
step(OzoneALL.reg)
