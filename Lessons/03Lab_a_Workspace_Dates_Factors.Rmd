...
filename <- here('Data','Raw','USGS_Site02085000_Flow_Raw.csv') # Commas can be replaced with / which allows you to keep it all within the same quotations
print(filename)
...
filenames <- dir(here('Data','Raw'),pattern = ("*.csv")) # * is a wild card that calls all the csv s in that directory
print(filenames)
```
...
ggplot(sites,aes(x=dow)) +
  geom_bar() + 
  scale_x_discrete(drop=FALSE) # Overrides the default which drops anything with zero inputs
...
sites$dow <- factor(sites$dow, levels=c("1","2","3","4","5","6","7")) #Or as.character(seq(1,7)) replaces levels=c...
