...
f1(score)
# PRINTED: Error in if (x > 50) { : the condition has length > 1

#13b. Run the second function with the vector of test scores
f2(score)
# PRINTED: "Pass" "Pass" "Pass" "Pass"
```

14. QUESTION: Which option of `if`...`else` vs. `ifelse` worked? Why? (Hint: search the web for "R vectorization")

> Answer: While ifelse()can handle vectorized operations, being capable of running multiple times for different values, if...else() cannot handle vectorized operations due to the fact that it can only run once for a singular value.
